...
county_results = select(county_expectations, state, FIPS) %>%
  mutate(dem_votes = as.numeric(NA), rep_votes = as.numeric(NA))

###sample results
county_results[county_results$FIPS==1001,]$dem_votes = 5000
county_results[county_results$FIPS==1001,]$rep_votes = 20000
```
